...
def get_scaffold(smiles: str) -> (str, str):
    def compress_mixture(smiles):
        return '.'.join(set(smiles.split('.')))
    ...
    non_generic = compress_mixture(Chem.MolToSmiles(non_generic))
    generic = compress_mixture(Chem.MolToSmiles(generic))
    return generic, non_generic


def get_data(cursor) -> dict:
    data = {}
    filter_phase_4 = "SELECT MOLREGNO, MESH_HEADING " \
    ...
    for indication in rows:
        molregno, mesh = indication

        drug_set.add(molregno)
        mesh_set.add(mesh)
    ...
    print(f'Drugs amount: {len(drug_set)}')
    print(f'Indications amount: {len(mesh_set)}')
    return data
